...
namespace MC3DSPluginFramework
{
    namespace Recipe
    {
        ...
        {
            return reinterpret_cast<gstd::vector<RecipeData *> *(*)()>(0x635CCC)();
        }

        void ResetList(gstd::vector<RecipeData *> *list)
        {
            reinterpret_cast<void (*)(gstd::vector<RecipeData *> *)>(0x638CC0)(list);
        }
        ...
        void SetUpRecipes(Regs &regs, u32 *sp, HookEx *hook)
        {
            ...
            for (auto &Append_Recipe : Append_Recipes) {
                ...
                for (auto &data : Append_Recipe.need) {
                    ItemInstance item(data.itemID, data.count, data.dataValue);

                    if (!data.enchants.empty())
                        item.setEnchants(gstd::vector<EnchantStatus>(data.enchants.begin(), data.enchants.end()));

                    need.push_back(std::move(item));
                }

                ItemInstance res(Append_Recipe.res.itemID, Append_Recipe.res.count, Append_Recipe.res.dataValue);

                if (!Append_Recipe.res.enchants.empty())
                    res.setEnchants(gstd::vector<EnchantStatus>(Append_Recipe.res.enchants.begin(), Append_Recipe.res.enchants.end()));
                ...
                RecipeData *newRecipe =
                    new RecipeData(
                        ...
                        std::move(need),
                        std::move(gstd::vector<ItemInstance>({ std::move(res) }))
                    );
        ...
        {
            static HookEx hook;
            hook.InitRoutine(0x47AF04, SetUpRecipes);
            hook.Enable();
