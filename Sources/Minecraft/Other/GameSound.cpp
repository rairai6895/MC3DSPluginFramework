...
void PlaySound(const Vec3_Float &pos, u32 id, u32 data1, u32 data2) {
    if (Entity *player = Player::GetInstance())
        if (gstd::vector<u32> *vec = (gstd::vector<u32> *)player->GetLevel()->offset(0x74))
            if (u32 SoundInstance = (*vec)[4])
                reinterpret_cast<void (*)(u32, u32, const Vec3_Float &, u32, u32)>(0x74A520)(SoundInstance, id, pos, data1, data2);
}
